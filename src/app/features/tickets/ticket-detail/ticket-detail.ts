...
export class TicketDetail implements OnInit {
  ...
  readonly ticketForm = this.fb.group({
    ...
    estado: this.fb.nonNullable.control<TicketEstado>(
      'Novo',
      Validators.required,
    ),
    prioridade: this.fb.nonNullable.control<TicketPrioridade>(
      'Normal',
      Validators.required,
    ),
    agendamento: this.fb.nonNullable.control<string>(''),
    descricao: this.fb.nonNullable.control<string>(''),
    userId: this.fb.nonNullable.control<string>(
      '',
      Validators.required,
    ),
  ...
  get contractRoute(): string[] | null {
    ...
    return getContractDetailRoute(
      this.ticket.companyId,
      this.ticket.contractId,
    );
  }

  get companyName(): string {
    return this.ticket
      ? getContractCompanyName(this.ticket.companyId)
      : '—';
  }
  ...
  get selectedTeams(): AssignableTicketTeam[] {
    return this.selectedTeamIds
      ...
      .filter(
        (team): team is AssignableTicketTeam => Boolean(team),
      );
  }
  ...
  saveChanges(): void {
    if (
      ...
      if (this.ticketForm.invalid) {
        this.showError(
          'Preencha os campos obrigatórios antes de guardar.',
        );
      }
    ...
    this.ticketService
      ...
      .subscribe({
        next: (updatedTicket) => {
          ...
          this.updateCanEditTicket();
          this.showSuccess(
            `O documento "${document.originalName}" foi removido com sucesso.`,
          );
        },
        error: (error: unknown) => {
          this.clearOwnSocketSuppression();
  ...
  downloadDocument(document: TicketDocument): void {
    ...
    this.ticketService
      .downloadDocument(this.ticket.id, document.fileName)
      .subscribe({
        next: (blob) => {
  ...
  getTeamPosition(team: TicketTeamDetail): string {
    ...
    const catalogTeam = this.teamCatalog.find(
      (entry) => entry.id === team.teamId,
    );

    return (
      catalogTeam?.positionList?.[team.minimumPositionIndex] ??
      `Nível ${team.minimumPositionIndex}`
    );
  }
  ...
  private loadSupportingData(): void {
    const currentUserRequest = this.currentUserId
      ? this.userService.getUserById(this.currentUserId).pipe(
          catchError(() => of<ProfileUser | null>(null)),
        )
  ...
  private updateCanEditTicket(): void {
    ...
    const currentUserTeams = this.resolveAssignableTeams(
      this.currentUserProfile,
    );

    this.canEditTicket = this.ticket.teams.some((ticketTeam) => {
      const currentTeam = currentUserTeams.find(
        (team) => team.id === ticketTeam.teamId,
      );

      return Boolean(
        currentTeam &&
          currentTeam.positionIndex <= ticketTeam.minimumPositionIndex,
      );
    });
  }
  ...
  private buildSnapshotFromForm(): TicketEditSnapshot {
    ...
    return {
      ...
      agendamento: value.agendamento
        ? new Date(value.agendamento).toISOString()
        : null,
  ...
  private buildPatchPayload(): UpdateTicketRequest {
    ...
    if (current.tipo !== original.tipo) {
      payload.tipo = current.tipo;
    }
  ...
  private resolveAssignableUsers(
    ...
    return activeUsers.filter((user) => {
      ...
      return this.getUserTeamIds(user).some((teamId) =>
        managedTeamSet.has(teamId),
      );
    });
  }

  private ensureCurrentResponsibleIsAssignable(): void {
    const responsible = this.ticket
      ? this.findUserById(this.ticket.userId)
      : null;

    if (
      responsible &&
      !this.assignableUsers.some((user) => user.id === responsible.id)
    ) {
  ...
  private resolveAssignableTeams(
    ...
    return rawTeams
      ...
      .map((team) => ({
        ...
        position:
          team.position?.trim() ||
          this.teamCatalog.find((entry) => entry.id === team.id)
            ?.positionList?.[team.positionIndex] ||
  ...
  private resolveTicketTeams(teamIds: string[]): TicketTeam[] {
    const currentTeams = new Map(
      (this.ticket?.teams ?? []).map((team) => [team.teamId, team]),
    );

    const selected = teamIds
      .map((teamId) => {
        const available = this.availableTeams.find(
          (team) => team.id === teamId,
        );
  ...
  private getManagedTeamIds(
    user: ProfileUser | null = this.currentUserProfile,
  ): string[] {
    if (!user) {
      return [];
    }
  ...
  private createFallbackAssignableTeam(
    teamId: string,
  ): AssignableTicketTeam | null {
    const ticketTeam = this.ticket?.teams.find(
  ...
  private areTeamsEqual(first: TicketTeam[], second: TicketTeam[]): boolean {
    return JSON.stringify(this.normalizeTicketTeams(first)) ===
    ...
    this.ticketService.getTicketById(this.ticketId).subscribe({
      next: (latestTicket) => {
        ...
        const primitiveKeys: Array<
          keyof Omit<TicketEditSnapshot, 'teams'>
        > = [
          'tipo',
          'estado',
          'prioridade',
        ...
        const userChangedTeams = !this.areTeamsEqual(
          currentSnapshot.teams,
        ...
        if (serverChangedTeams) {
          if (!userChangedTeams) {
            ...
            !this.areTeamsEqual(currentSnapshot.teams, latestSnapshot.teams)
          ) {
            conflicts += 1;
          }
        }

        const userChangedResponsible =
          currentSnapshot.userId !== originalSnapshot.userId;
        const serverChangedResponsible =
          latestSnapshot.userId !== originalSnapshot.userId;
  ...
  private setFormSnapshotValue(
    ...
    switch (key) {
      case 'tipo':
        this.ticketForm.controls.tipo.setValue(value as TicketTipo);
        break;
      case 'estado':
        this.ticketForm.controls.estado.setValue(value as TicketEstado);
        break;
      case 'prioridade':
        this.ticketForm.controls.prioridade.setValue(
          value as TicketPrioridade,
        );
        break;
      case 'agendamento':
        this.ticketForm.controls.agendamento.setValue(
          this.toDateTimeLocal(value as string | null),
        );
        break;
      case 'descricao':
