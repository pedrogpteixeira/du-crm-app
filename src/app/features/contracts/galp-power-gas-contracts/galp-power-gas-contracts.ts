...
export class GalpPowerGasContracts implements OnInit {
  ...
  contracts: GalpPowerGasContractList[] = [];
  filteredContracts: GalpPowerGasContractList[] = [];
  contractsByStatus: Record<string, GalpPowerGasContractList[]> = {};
  ...
  readonly statuses: GalpPowerGasContractStatus[] = [...GALP_POWER_GAS_STATUSES];
  ...
  loadMore(status: GalpPowerGasContractStatus): void {
  ...
  hasMoreForStatus(status: GalpPowerGasContractStatus): boolean {
    return this.paginationByStatus[status]?.hasMore ?? false;
  }

  isLoadingStatus(status: GalpPowerGasContractStatus): boolean {
  ...
  getContractsByStatus(status: GalpPowerGasContractStatus): GalpPowerGasContractList[] {
  ...
  private replaceWithResponse(response: ContractKanbanResponse<GalpPowerGasContractList>): void {
    this.totalContracts = Number.isFinite(response.total) ? Math.max(0, response.total) : 0;

    const contractsByStatus: Record<string, GalpPowerGasContractList[]> = {};
  ...
  private handleSocketContract(
    ...
    let existing: GalpPowerGasContractList | undefined;
    ...
    const nextStatus = String(event.estado ?? existing?.estado ?? '') as GalpPowerGasContractStatus;
    ...
    const updatedContract = {
      ...
      id: contractId,
      estado: nextStatus,
    } as GalpPowerGasContractList;

    const nextColumns: Record<string, GalpPowerGasContractList[]> = {};
  ...
  getContractUserName(contract: GalpPowerGasContractList): string {
