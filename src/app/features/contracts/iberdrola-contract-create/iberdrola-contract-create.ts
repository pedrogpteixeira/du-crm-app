...
export class IberdrolaContractCreate implements OnInit {
  ...
  readonly iberdrolaCompanyId = IBERDROLA_COMPANY_ID;
  ...
  createContract(): void {
    ...
    let createdContract: IberdrolaContractDetail | null = null;

    this.iberdrolaContractService
      .createIberdrolaContract(payload)
      .pipe(
        tap((contract) => {
      ...
      .subscribe({
        next: (contract) => {
          ...
          this.router.navigate(['/home/contracts/iberdrola', contract.id]);
        },
  ...
  private loadCampaigns(): void {
    this.campaignService
      .getCampaignsByCompanyId(IBERDROLA_COMPANY_ID)
  ...
  private resolveContractTeams(): IberdrolaContractTeamVisibility[] {
  ...
  private buildContractPayload(): CreateIberdrolaContractRequest {
    if (!this.client) {
    ...
    const estado: IberdrolaContractStatus = this.estadoOptions[0];

    const payload: CreateIberdrolaContractRequest = {
      companyId: IBERDROLA_COMPANY_ID,
  ...
  private addIfFilled<Key extends keyof CreateIberdrolaContractRequest>(
    payload: CreateIberdrolaContractRequest,
    key: Key,
    value: CreateIberdrolaContractRequest[Key] | '' | null | undefined,
  ): void {
    if (value === null || value === undefined || value === '') {
      return;
    }

    payload[key] = value as CreateIberdrolaContractRequest[Key];
  }
