...
interface WallboxContractApiShape extends WallboxContractDetailModel {
...
@Component({
  ...
  templateUrl: './wallbox-contract-detail.html',
  changeDetection: ChangeDetectionStrategy.Eager,
  styleUrl: './wallbox-contract-detail.scss',
})
export class WallboxContractDetail implements OnInit {
  ...
  contract: WallboxContractDetailModel | null = null;
  ...
  readonly tipoLocalInstalacaoOptions: WallboxTipoLocalInstalacao[] = [
    'Moradia',
    'Condomínio Ligação a QE comum',
    'Condomínio Ligação a QE cliente',
  ];

  readonly metodoPagamentoOptions: WallboxMetodoPagamento[] = [
    'Pronto Pagamento',
  ...
  ngOnInit(): void {
    ...
    this.socketService.listenWallboxContractUpdated().subscribe((event) => {
  ...
  loadContract(contractId: string, loadCampaignOptions = false): void {
    ...
    this.wallboxContractService
      .getWallboxContractById(contractId)
      .pipe(
        finalize(() => {
  ...
  private submitObservationValue(message: string, internal: boolean): void {
    ...
    this.wallboxContractService
      .updateWallboxContract(this.contractId, payload)
      .pipe(
        finalize(() => {
          if (internal) {
  ...
  saveChanges(): void {
    ...
    const updateRequest: Observable<WallboxContractDetailModel> = hasContractChanges
      ? this.wallboxContractService.updateWallboxContract(this.contractId, payload)
      : of(this.contract);
  ...
  isDeletingAttachment(document: WallboxContractDocument): boolean {
  ...
  private refreshContractActivity(): void {
    ...
    this.wallboxContractService.getWallboxContractById(this.contractId).subscribe({
      next: (latestContract) => {
  ...
  private mergeExternalContract(latestContract: WallboxContractDetailModel): {
    updated: number;
    conflicts: number;
  } {
  ...
  private hasContractAccess(contract: WallboxContractDetailModel): boolean {
  ...
  private normalizeContractResponse(
    ...
    const apiContract = contract as WallboxContractApiShape;
  ...
  private initializeEditForm(contract: WallboxContractDetailModel): void {
  ...
  private buildEditableState(contract: WallboxContractDetailModel): {
    form: EditableContractForm;
    ...
    return {
      ...
      form: {
        ...
        tipoLocalInstalacao: contract.tipoLocalInstalacao ?? '',

        metodoPagamento: contract.metodoPagamento ?? '',

        comDeslocacao: Boolean(contract.comDeslocacao),

        balanceamentoPotencia: Boolean(contract.balanceamentoPotencia),
  ...
  private resolveCampaignSelection(campaign: WallboxContractDetailModel['campaign']): {
    mode: CampaignSelectionMode;
    campaignId: string;
    customCampaign: string;
  } {
  ...
  private buildPatchPayload(): UpdateWallboxContractRequest {
    const payload: UpdateWallboxContractRequest = {};
    ...
    this.assignChangedValue(
      payload,
      'tipoLocalInstalacao',
      this.editForm.tipoLocalInstalacao,
      this.originalEditForm.tipoLocalInstalacao,
    );
    ...
    this.assignChangedValue(
      payload,
      'comDeslocacao',
      this.editForm.comDeslocacao,
      this.originalEditForm.comDeslocacao,
    );

    this.assignChangedValue(
      payload,
      'balanceamentoPotencia',
      this.editForm.balanceamentoPotencia,
      this.originalEditForm.balanceamentoPotencia,
    );
  ...
  private assignChangedValue(
    ...
    key: keyof UpdateWallboxContractRequest,
  ...
  getSegmentLabel(segment: WallboxTipoSegmento): string {
    return this.tipoSegmentoOptions.find((option) => option.value === segment)?.label ?? segment;
  }

  canPreviewDocument(file: WallboxContractDocument): boolean {
    return this.documentPreview.canPreview(file);
  }

  previewDocument(file: WallboxContractDocument): void {
    if (!this.contract?.id) {
      return;
    }

    this.documentPreview
      .preview(file, () => this.wallboxContractService.downloadDocument(this.contract!.id, file))
      .subscribe({
        error: () => this.showError('Não foi possível pré-visualizar o anexo.'),
      });
  }

  downloadDocument(file: WallboxContractDocument): void {
    ...
    this.wallboxContractService.downloadDocument(this.contract.id, file).subscribe({
      next: (blob) => {
