...
import {
  ...
  PortulogosContractTeamVisibility,
...
export class PortulogosContractCreate implements OnInit {
  ...
  estadoOptions: readonly PortulogosContractStatus[] = PORTULOGOS_CONTRACT_STATUSES;
  ...
  contractForm = {
    ...
    estado: 'Pedido de Chamada' as PortulogosContractStatus,
  ...
  private resolveContractTeams(): PortulogosContractTeamVisibility[] {
  ...
  private loadCampaigns(): void {
    this.campaignService
      .getCampaignsByCompanyId(environment.PORTULOGOS_COMPANY_ID)
  ...
  createContract(): void {
    ...
    let createdContract: PortulogosContractDetail | null = null;

    this.portulogosContractService
      .createPortulogosContract(payload)
      .pipe(
        tap((contract) => {
        ...
        switchMap((contract) => {
          ...
          return this.portulogosContractService.uploadAttachments(contract.id, this.selectedFiles).pipe(
            tap(() => {
              this.isUploadingDocuments = false;
            }),
      ...
      .subscribe({
        next: (contract) => {
          ...
          this.router.navigate(['/home/contracts/portulogos', contract.id]);
        },
        ...
        error: (error) => {
          this.errorMessage =
            ...
            error?.error?.message ||
            'Não foi possível criar o contrato Portulogos.';
        },
  ...
  private buildContractPayload(): CreatePortulogosContractRequest {
    ...
    const estado: PortulogosContractStatus = this.estadoOptions[0];

    const payload: CreatePortulogosContractRequest = {
  ...
  private addLightRegistrationFields(payload: CreatePortulogosContractRequest): void {
  ...
  private addSharedFields(payload: CreatePortulogosContractRequest): void {
  ...
  private addProFields(payload: CreatePortulogosContractRequest): void {
    if (this.shouldShowLuzFields()) {
  ...
  private addBoolean<T extends object>(payload: T, key: keyof T, value: boolean): void {
    (payload as Record<string, unknown>)[key as string] = value;
  }
