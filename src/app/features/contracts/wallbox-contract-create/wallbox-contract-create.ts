...
type WallboxMetodoPagamento = 'Pronto Pagamento' | 'Pagamento em Prestações';

type WallboxMoradaFaturacaoSelecao = 'Igual à de Instalação' | 'Outra';
...
export class WallboxContractCreate implements OnInit {
  ...
  readonly moradaFaturacaoOptions: WallboxMoradaFaturacaoSelecao[] = [
  ...
  contractForm = {
    ...
    tipoSegmento: 'Residencial' as WallboxTipoSegmento,

    tipoProduto: 'Luz' as WallboxTipoProduto,

    estado: 'Pedido de Chamada' as WallboxContractStatus,
    ...
    moradaFaturacaoSelecao: 'Igual à de Instalação' as WallboxMoradaFaturacaoSelecao,
    ...
    nivelTensao: '' as WallboxNivelTensao | '',

    tipoLocalInstalacao: '' as WallboxTipoLocalInstalacao | '',

    metodoPagamento: '' as WallboxMetodoPagamento | '',
  ...
  cancel(): void {
    ...
    this.router.navigate(['/home/contracts/wallbox']);
  }
    ...
    this.wallboxContractService
      ...
      .subscribe({
        next: (contract: WallboxContractCreateResponse) => {
          ...
          this.router.navigate(['/home/contracts/wallbox', contract.id]);
        },
        ...
        complete: () => {
          if (createdContractId && this.errorMessage.includes('O contrato foi criado')) {
            this.successMessage = `Contrato ${createdContractId} criado com sucesso.`;
          }
        },
      });
  }
  ...
  private loadCampaigns(): void {
    this.campaignService
      .getCampaignsByCompanyId(WALLBOX_COMPANY_ID)
  ...
  private resolveContractTeams(): WallboxContractTeamVisibility[] {
  ...
  private buildContractPayload(): CreateWallboxContractRequest {
    if (!this.client) {
    ...
    const estado: WallboxContractStatus = this.estadoOptions[0];

    const payload: CreateWallboxContractRequest = {
      companyId: WALLBOX_COMPANY_ID,
      ...
      comDeslocacao: this.contractForm.comDeslocacao,
      balanceamentoPotencia: this.contractForm.balanceamentoPotencia,
    };
    ...
    this.addIfFilled(payload, 'tipoLocalInstalacao', this.contractForm.tipoLocalInstalacao);
