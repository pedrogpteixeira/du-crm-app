...
export class RepsolContractCreate implements OnInit {
  ...
  estadoOptions: readonly RepsolContractStatus[] = REPSOL_CONTRACT_STATUSES;
  ...
  private resolveContractTeams(): RepsolContractTeamVisibility[] {
    ...
    this.repsolContractService
      ...
      .pipe(
        ...
        switchMap((contract) => {
          ...
          return this.repsolContractService.uploadAttachments(contract.id, this.selectedFiles).pipe(
            tap(() => {
              this.isUploadingDocuments = false;
            }),
  ...
  private buildContractPayload(): CreateRepsolContractRequest {
    if (!this.client) {
      throw new Error('Cliente não identificado.');
    }

    const estado: RepsolContractStatus = this.estadoOptions[0];

    const payload: CreateRepsolContractRequest = {
  ...
  private addProFields(payload: CreateRepsolContractRequest): void {
    if (this.shouldShowLuzFields()) {
